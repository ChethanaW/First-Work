...
public final class Complex {
	...
	public Complex multiply(Complex c) {
		...
		Complex a = this;
		double real = (a.r * c.r) - (a.i * c.i);
		double imag = (a.r * c.i) + (a.i * c.r);
		return new Complex(real,imag);
	}
	...
	public static Complex valueOf(String s) {
		...
		double first = Double.parseDouble(parts.get(0));
		String thirdnum = parts.get(2).substring(0, parts.get(2).length()-1);
		double third = Double.parseDouble(thirdnum);
		
		if (parts.get(1).contains("-")){
			third = third * -1;
		}
		
		return new Complex (first, third) ;
	}
