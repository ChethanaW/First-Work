...
public class Die {
	...
	public Die(Die other) {
		this.valueMap = new TreeMap<>(other.valueMap);
		this.curface = other.curface;
	...
	public String roll() {
		...
		this.curface = rn.nextInt(this.getNumberOfFaces()) + 1;
		return this.valueMap.get(this.curface);
	...
	public SortedMap<Integer, String> getValueMap() {
		//SortedMap<Integer, String> sortedData = new TreeMap<>();

		//for (int n = 1; n <= this.getNumberOfFaces(); n++) {
			//this.curface = n;
			//sortedData.put(n, this.valueMap.get(n));
		//}
	...
	public int hashCode() {
		...
		for (int n = 1; n <= this.getNumberOfFaces(); n++) {
			this.curface = n;
			int num = this.getValue().hashCode();
			sum = sum + num;
		}
		return sum;
	...
	public boolean equals(Object obj) {
		...
		String [] l1 = new String[other.getNumberOfFaces()];
		other.getValueMap().values().toArray(l1);
		String [] l2 = new String[this.getNumberOfFaces()];
		this.getValueMap().values().toArray(l2);
		Arrays.sort(l1);
		Arrays.sort(l2);
		
		if (!(Arrays.equals(l1, l2)) && this.getValue().equals(other.getValue())) {
		 ...
		 * 
		 * if (this.getValueMap().containsValue(other.getValue())){ if
		 * (other.getValueMap().containsValue(this.getValue())){ return true; }
		 * 
		 * }
		 * 
		 * if (this.getValue().equals(other.getValue())){ return true; } if
		 * (this.curface == other.curface){ return true; }
		 * 
		 * 
		 * 
		 * }
		 ...
		 * if (this.valueMap.get(this.curface).equals(other.valueMap.get(other.
		 * curface))){ return true; }
		 * 
		 * if (this.valueMap.size() == other.valueMap.size()){ for (int n= 1 ;
		 * n< this.valueMap.size() ; n++){ this.curface = n; if
		 * (other.valueMap.containsValue(this.valueMap.get(n))){ return true; }
		 * }
		 * 
		 * } /* if (this.curface == other.curface){ return true; }
		 * 
		 * if (this.getValue().equals(other.getValue())){ return true; }
		 * 
		 * 
		 * all these saved because i need to know why these doesn't work, need to ask the professor.
		 * 
		 * 
		 */
	...
	public String toString() {
		String a = " " + this.getValueMap().get(1) + ", ";
		for (int n = 2; n <= this.getNumberOfFaces(); n++) {
			String b = this.getValueMap().get(n) + ", ";
			a = a + b;
		}
